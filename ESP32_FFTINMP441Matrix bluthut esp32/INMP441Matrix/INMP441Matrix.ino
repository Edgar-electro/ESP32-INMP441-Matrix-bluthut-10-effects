...
cLEDMatrix<M_WIDTH, M_HEIGHT, VERTICAL_ZIGZAG_MATRIX> leds;
...
void rainbowBars(uint8_t band, uint8_t barHeight) {
  ...
  for (int x = xStart; x < xStart + barWidth; x++) {
    for (int y = 0; y <= barHeight; y++) {
      leds(x,y) = CHSV((y / barWidth) * (255 / numBands), 255, 255);
    }
  }
}


void rainbowBars1(uint8_t band, uint8_t barHeight) {
  int xStart = barWidth * band;
  for (int x = xStart; x < xStart + barWidth; x++) {
    for (int y = 0; y <= barHeight; y++) {
